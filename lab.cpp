...
Time::
    ...
    {
        this->setUnix64Time(self, u_time + UNIX_TIME_OFFSET);
        return;
    }
Time::
    setFileTime(uint64_t filetime)
    {
        this->setUnixTime(filetime / WINDOWS_TICK - SEC_TO_UNIX_EPOCH);
        return;
    }
Time::
...
Time::
    getFileTime() const
    {
        if ((this->unixtime * WINDOWS_TICK + SEC_TO_UNIX_EPOCH) > WINDOWS_MAX_TIME)
        {
            return WINDOWS_MAX_TIME
        }
        else 
        {
            return (this->unixtime * WINDOWS_TICK + SEC_TO_UNIX_EPOCH);
        }
    }
Time:: 
    getFull() const
...
std::ostream& Time::operator<< (std::ostream &out, const Time& right)
{
...
std::istream& Time::operator>> (std::istream &in, Time& right)
{
    unsigned int day = 0;
...
Time Time::operator +(Time const& left, Time const& right) 
{
    try ()
    {
...
Time& Time::operator +=(Time const& left, Time const& right) 
{
    uint64_t l_u_time = left->getUnix64Time();
    uint64_t r_u_time = right->getUnix64Time();
    try ()
    {
        left->setUnix64Time(l_u_time += r_u_time);
    }
    catch (...)
    {
        std::cerr << "UnixDate is out of range, UnixDate must be smaller than 18446744073709551615" << std::endl;
...
bool Time::operator ==(const Time& left, const Time& right)
{
    return (left->getUnix64Time() == right->getUnix64Time());
}
bool Time::operator !=(const Time& left, const Time& right)
